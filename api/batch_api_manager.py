...
class BatchAPIManager:
    ...
    async def batch_multi_price(self, addresses: List[str], scan_id: Optional[str] = None) -> Dict[str, Any]:
        """
        FIXED: Use real batch endpoints for efficient price fetching.
        Utilizes the /defi/multi_price endpoint which supports up to 100 tokens per call.
        ...
        if hasattr(self.birdeye_api, '_filter_solana_addresses'):
            ...
            if filtered_addresses:
                self.logger.info(f"🔍 Batch price: Filtered out {len(filtered_addresses)} non-Solana addresses")
        ...
        # Use proper batch processing with multi_price endpoint
        batch_size = 100  # Max batch size for multi_price endpoint
        all_price_data = {}
        ...
        for i in range(0, len(unique_addresses), batch_size):
            ...
            self.structured_logger.info({
                "event": "batch_price_call", 
                "endpoint": "/defi/multi_price", 
                "batch_size": len(batch_addresses), 
                "scan_id": scan_id
            })
            ...
            try:
                ...
                # Only fetch uncached addresses using batch endpoint
                if uncached_addresses:
                    batch_prices = await self.birdeye_api.get_multi_price(uncached_addresses)
                    
                    if batch_prices and batch_prices.get('success'):
                        data = batch_prices.get('data', {})
                        for address in uncached_addresses:
                            if address in data:
                                # Cache individual results
                                cache_key = f"price_{address}"
                                self.cache_manager.set(cache_key, data[address], ttl=30)  # 30s TTL for prices
                                all_price_data[address] = data[address]
                
                # Add cached results
                all_price_data.update(cached_results)
                
            except Exception as e:
                self.logger.warning(f"Batch price fetch failed for batch {i//batch_size + 1}: {e}")
                # Fallback to individual calls only if batch fails
                await self._fallback_individual_prices(batch_addresses, all_price_data)
        
        # Track API usage efficiency - now with REAL batching
        actual_api_calls = len([batch for batch in range(0, len(unique_addresses), batch_size)])
        individual_calls_would_be = len(unique_addresses)
        calls_saved = individual_calls_would_be - actual_api_calls
        
        self.ultra_stats['total_calls_made'] += actual_api_calls
        self.ultra_stats['total_calls_saved'] += calls_saved
        
        self.structured_logger.info({
            "event": "batch_price_complete", 
            "endpoint": "/defi/multi_price", 
            "total_addresses": len(unique_addresses),
            "successful": len(all_price_data),
            "api_calls_made": actual_api_calls,
            "api_calls_saved": calls_saved,
            "batch_efficiency": f"{(calls_saved/individual_calls_would_be)*100:.1f}%",
            "scan_id": scan_id
        })
        
        self.logger.info(f"🚀 REAL Batch prices: {len(all_price_data)}/{len(unique_addresses)} successful, {calls_saved} API calls saved ({(calls_saved/individual_calls_would_be)*100:.1f}% efficiency)")
        return all_price_data

    async def _fallback_individual_prices(self, addresses: List[str], results_dict: Dict[str, Any]) -> None:
        """Fallback to individual price calls when batch fails."""
        for address in addresses:
            try:
                await asyncio.sleep(0.2)  # Rate limit individual calls
                price_data = await self.birdeye_api.get_token_price(address)
                if price_data:
                    results_dict[address] = price_data
                    # Cache individual result
                    cache_key = f"price_{address}"
                    self.cache_manager.set(cache_key, price_data, ttl=30)
            except Exception as e:
                self.logger.warning(f"Individual price fallback failed for {address}: {e}")
    
    async def batch_token_overviews(self, addresses: List[str], scan_id: Optional[str] = None) -> Dict[str, Any]:
        """
        FIXED: Use real batch endpoints for efficient token metadata fetching.
        The batch endpoint IS available for Starter Plan and provides 60-90% cost reduction.
        ...
        # Use proper batch processing with batch endpoint
        batch_size = 50  # Max batch size for metadata endpoint
        all_metadata = {}
        ...
        for i in range(0, len(unique_addresses), batch_size):
            ...
            try:
                ...
                # Only fetch uncached addresses using batch endpoint
                if uncached_addresses:
                    batch_metadata = await self.birdeye_api.get_token_metadata_multiple(uncached_addresses, scan_id)
                    
                    if batch_metadata and batch_metadata.get('success'):
                        data = batch_metadata.get('data', {})
                        for address in uncached_addresses:
                            if address in data:
                                # Cache individual results
                                cache_key = f"metadata_{address}"
                                self.cache_manager.set(cache_key, data[address], ttl=300)
                                all_metadata[address] = data[address]
                
                # Add cached results
                all_metadata.update(cached_results)
                
            except Exception as e:
                self.logger.warning(f"Batch metadata fetch failed for batch {i//batch_size + 1}: {e}")
                # Fallback to individual calls only if batch fails
                await self._fallback_individual_metadata(batch_addresses, all_metadata)
        ...
        self.logger.info(f"🚀 REAL Batch metadata: {len(all_metadata)}/{len(unique_addresses)} successful, {calls_saved} API calls saved ({(calls_saved/individual_calls_would_be)*100:.1f}% efficiency)")
        return all_metadata

    async def _fallback_individual_metadata(self, addresses: List[str], results_dict: Dict[str, Any]) -> None:
        """Fallback to individual metadata calls when batch fails."""
        for address in addresses:
            try:
                await asyncio.sleep(0.2)  # Rate limit individual calls
                metadata = await self.birdeye_api.get_token_metadata_single(address)
                if metadata:
                    results_dict[address] = metadata
                    # Cache individual result
                    cache_key = f"metadata_{address}"
