...
class BirdeyeAPI:
    ...
    def __init__(self, 
        ...
        self.debug_mode = debug_mode or config.get('debug', False)
        ...
        # Create API session context
        self.api_session_id = self.enhanced_logger.new_scan_context(
            strategy="birdeye-api-session",
            timeframe="session",
        )
        ...
        # Log initialization info with structured logging
        self.enhanced_logger.info("🚀 BirdeyeAPI initialized successfully",
                                base_url=self.base_url,
                                api_key_status="loaded" if self.api_key else "not_set",
                                rate_limit_rpm=self.rate_limit,
                                request_timeout_seconds=self.config.get('request_timeout_seconds', 20),
        ...
        if self.debug_mode:
            self.enhanced_logger.debug("🐛 BirdEye debug mode activated",
                                     debug_features=["api_call_tracing", "response_logging", "performance_monitoring"])
        ...
        try:
            ...
            self.enhanced_logger.info("🧮 BirdEye cost calculator initialized",
                                    cost_tracking_enabled=True,
                                    compute_units_monitoring=True)
            self.logger.info("🧮 BirdEye cost calculator initialized - tracking compute units")
        except ImportError as e:
            self.enhanced_logger.warning("BirdEye cost calculator not available",
                                       error=str(e),
                                       cost_tracking_enabled=False)
            self.logger.warning(f"BirdEye cost calculator not available: {e}")
            self.cost_calculator = None
            self.enhanced_logger.info("BirdEye cost calculator disabled", cost_tracking_enabled=False)
    ...
    async def _make_request(self, endpoint: str, params: Optional[Dict[str, Any]] = None, custom_headers: Optional[Dict[str, str]] = None, scan_id: Optional[str] = None) -> Any:
        ...
        try:
            ...
            async with session.get(url, params=params, headers=request_headers) as response:
                ...
                # Track API call metrics with enhanced structured logging
                self.enhanced_logger.info("API response received",
                ...
                if self.debug_mode:
                    self.enhanced_logger.debug("API call metrics updated",
                                             total_calls=self.api_call_tracker['total_api_calls'],
                                             success_rate=self.api_call_tracker['successful_api_calls'] / max(1, self.api_call_tracker['total_api_calls']))
                
                # Legacy structured logging for backward compatibility  
                log_data = {
                    "event": "api_call",
                    "endpoint": endpoint,
                    "params": params,
