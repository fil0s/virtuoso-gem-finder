#!/usr/bin/env python3
"""
48-Hour High Conviction Token Detector - Pure Scoring System
Runs scans every 30 minutes for 48 hours (96 total cycles)
...
async def run_48hour_detector():
    """Run detector for 48 hours with 30-minute intervals"""
    
    # Configuration
    TOTAL_DURATION_HOURS = 48
    INTERVAL_MINUTES = 30
    TOTAL_CYCLES = TOTAL_DURATION_HOURS * 2  # 2 scans per hour
    
    print("🎯 HIGH CONVICTION TOKEN DETECTOR - 48 HOUR SESSION")
    print("=" * 60)
    ...
    try:
        ...
        while time.time() < session_end and cycle_number <= TOTAL_CYCLES:
            ...
            try:
                ...
                total_api_calls = 0
                api_stats = {'birdeye': 0, 'dexscreener': 0, 'rugcheck': 0, 'jupiter': 0, 'meteora': 0}
        ...
        try:
            if hasattr(detector, 'session_stats') and detector.session_stats:
                ...
                # Extract detailed stats for each platform (including Jupiter & Meteora)
                for platform in ['birdeye', 'dexscreener', 'rugcheck', 'jupiter', 'meteora']:
...
if __name__ == '__main__':
    try:
        success = asyncio.run(run_48hour_detector())
        if success:
            print("\n🎉 48-hour session completed successfully!")
        else:
            print("\n💥 48-hour session failed!")
            sys.exit(1)
    except KeyboardInterrupt:
        print("\n⚠️ Session interrupted by user")
        sys.exit(0)
