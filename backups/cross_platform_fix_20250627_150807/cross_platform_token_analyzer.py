...
class MeteoraConnector:
    ...
    async def get_volume_trending_pools(self, limit: int = 50) -> List[Dict]:
        ...
        try:
            ...
            # Since most early pairs are SOL/USDC, we need to check more pairs
            max_pairs_to_check = min(len(all_pairs), limit * 10)  # Check 10x more pairs
            tokens_found = 0
            ...
            for pool in all_pairs[:max_pairs_to_check]:
                ...
                # For Meteora, we want to find the non-excluded token in the pair
                # Most pairs will have SOL/USDC as one side, we want the other side
                target_mints = []
                ...
                # If both tokens are excluded or no valid tokens, skip this pool
                if not target_mints:
                    continue
...
class CrossPlatformAnalyzer:
    ...
    async def collect_all_data(self) -> Dict[str, List[Dict]]:
        ...
        tasks.append(('orca_trending_pools', self.orca.get_trending_pools(min_volume_24h=5000)))
        tasks.append(('raydium_trending_pairs', self.raydium.get_volume_trending_pairs(min_volume_24h=5000)))
    ...
    def normalize_token_data(self, platform_data: Dict[str, List[Dict]]) -> Dict[str, Dict]:
        ...
        # Process Orca trending pools (DEX liquidity data)
        ...
        # Process Raydium trending pairs (DEX liquidity data)
    ...
    def _calculate_token_score(self, token_data: Dict) -> float:
        ...
        # Cross-Platform Validation Score - Only cross-validated tokens get points
        # Only award points for cross-platform validation (2+ platforms)
        if platforms >= 2:
            score += (platforms - 1) * 8.0  # 2 platforms=8, 3=16, 4=24, 5=32
        ...
        # Emerging Star Discovery Bonus - Max: 15 points
        if 'birdeye_emerging_stars' in token_data['platforms'] and 'birdeye' in token_data['data']:
            be_data = token_data['data']['birdeye']
            
            # Emerging star discovery bonus - 5 points
            if be_data.get('is_emerging_star', False):
                score += 5.0
            
            # Volume-to-market-cap momentum scoring - 0-10 points
            volume_to_mcap_ratio = be_data.get('volume_to_mcap_ratio', 0)
            if volume_to_mcap_ratio >= 100.0:
                score += 10.0  # Extreme momentum (potential moonshot)
            elif volume_to_mcap_ratio >= 10.0:
                score += 8.0   # Very high momentum
            elif volume_to_mcap_ratio >= 5.0:
                score += 6.0   # High momentum
            elif volume_to_mcap_ratio >= 2.0:
                score += 4.0   # Medium momentum
            elif volume_to_mcap_ratio >= 1.0:
                score += 2.0   # Low momentum
            
            # Emerging star score integration - use pre-calculated score
            emerging_star_score = be_data.get('emerging_star_score', 0)
            if emerging_star_score >= 80.0:
                score += 8.0   # Exceptional emerging star
            elif emerging_star_score >= 60.0:
                score += 6.0   # Strong emerging star
            elif emerging_star_score >= 40.0:
                score += 4.0   # Good emerging star
            elif emerging_star_score >= 20.0:
                score += 2.0   # Decent emerging star
        ...
        # Jupiter Emerging Token Scoring - Max: 15 points
        if 'jupiter' in token_data['data']:
            jp_data = token_data['data']['jupiter']
            
            # Risk level scoring (lower risk = higher score) - 0-5 points
            risk_level = jp_data.get('risk_level', 'HIGH')
            if risk_level == 'LOW':
                score += 5.0
            ...
            # Liquidity analysis bonus - 0-8 points
            if jp_data.get('has_liquidity_analysis', False):
                liquidity_score = jp_data.get('liquidity_score', 0)
                price_impact = jp_data.get('price_impact_pct', 100)  # Default high impact
                
                # High liquidity score bonus
                ...
                # Low price impact bonus (better liquidity)
                if price_impact <= 1.0:  # <1% price impact
                    score += 2.0
                elif price_impact <= 2.5:  # <2.5% price impact
                    score += 1.0
            
            # Token maturity and legitimacy - 0-2 points
            if jp_data.get('logo_uri'):
                score += 1.0  # Has logo
            if jp_data.get('tags') and len(jp_data.get('tags', [])) > 0:
                score += 1.0  # Has tags/categories
        
        # Meteora Pool-Based Scoring - Max: 12 points
        if 'meteora' in token_data['data']:
            mt_data = token_data['data']['meteora']
            
            # Volume-to-Liquidity Ratio (VLR) scoring - 0-8 points
            vlr = mt_data.get('vlr', 0)
            if vlr >= 2.0:
                score += 8.0  # Very high activity
            elif vlr >= 1.0:
                score += 6.0  # High activity
            elif vlr >= 0.5:
                score += 4.0  # Medium activity
            elif vlr >= 0.1:
                score += 2.0  # Low activity
            
            # Meteora score integration - 0-4 points
            meteora_score = mt_data.get('meteora_score', 0)
            if meteora_score >= 80:
                score += 4.0
            elif meteora_score >= 60:
                score += 3.0
            elif meteora_score >= 40:
                score += 2.0
            elif meteora_score >= 20:
                score += 1.0
        
        # Pure scoring without categories - just return the calculated score
        adjusted_score = score
        # Store final score in metadata
        if 'metadata' not in token_data:
            token_data['metadata'] = {}
        token_data['metadata']['final_score'] = round(adjusted_score, 1)
