...
MAJOR_TOKENS_TO_EXCLUDE = {
    ...
    '9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL9zYtAWWM',  # MATIC
    ...
    # Additional major Solana ecosystem tokens
...
class EarlyTokenDetector:
    ...
    def __init__(self, config: Optional[Dict] = None, enable_whale_tracking: bool = False):
        ...
        # Initialize whale activity analyzer with discovery service
        self.whale_analyzer = WhaleActivityAnalyzer(
            logger=self.logger, 
    ...
    async def _calculate_quick_scores(self, tokens: List[Dict], basic_metrics: Dict[str, Dict]) -> List[Tuple[Dict, float]]:
        ...
        for token in tokens:
            ...
            # Age score (20%) - newer is better for early detection
        ...
        try:
            ...
            if creation_time:
                ...
                if 1 <= age_hours <= 24:
                    ...
                    scores['age'] = 15
                elif 72 < age_hours <= 168:  # 1 week
                    scores['age'] = 12
                elif age_hours <= 1:
                    scores['age'] = 5  # Too new
                else:
                    scores['age'] = 8  # Too old
                    
                self.logger.debug(f"[SCORING] {token_symbol} - Age: {age_hours:.1f}h -> Score: {scores['age']}/20")
            else:
                scores['age'] = 10
                self.logger.debug(f"[SCORING] {token_symbol} - Age: Unknown -> Default Score: {scores['age']}/20")
    ...
    async def perform_whale_analysis(self, token_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Perform whale analysis with error handling for dict issues.
        
        Args:
            token_data: Token data including address
        ...
        if not token_address:
            self.logger.warning("[WHALE] Missing token address for whale analysis")
            return self._default_whale_result()
        
        try:
            self.logger.info(f"[WHALE] Starting whale activity analysis for {token_data.get('symbol', 'UNKNOWN')}")
            
            # Get token holders
            holder_data = await self.birdeye_api.get_token_holders(token_address, limit=100)
            
            if not holder_data or 'items' not in holder_data:
                self.logger.warning(f"[WHALE] No holder data for {token_address}")
                return self._default_whale_result()
            
            # Process holder data safely
            processed_holders = []
            for holder in holder_data.get('items', []):
                ...
                if isinstance(holder, dict):
                    ...
                    # Handle non-dict holder data
                    self.logger.warning(f"[WHALE] Unexpected holder data type: {type(holder)}")
            
            # Sort by value
            processed_holders.sort(key=lambda h: h['value_usd'], reverse=True)
            
            # Identify whales (holders with >1% of supply)
            total_value = sum(h['value_usd'] for h in processed_holders)
            whale_threshold = total_value * 0.01
            whales = [h for h in processed_holders if h['value_usd'] >= whale_threshold]
            
            # Analyze whale activity
            result = self._analyze_whale_movements(whales, token_address)
            return result
