...
Tests all available APIs against specific tokens to compare:
- Data availability and quality
- Response times and reliability
- Coverage across different APIs
- Performance metrics and insights

This script tests the tokens from the user's analysis table across all available APIs.
"""
...
class ComprehensiveAPIComparison:
    ...
    async def initialize_apis(self):
        ...
        try:
            ...
            self.logger.info(f"✅ Initialized {len(self.apis)} API connectors")
    ...
    async def test_birdeye_api(self, token_address: str, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            price_data = await self.apis["birdeye"].get_token_price(token_address)
            volume_data = await self.apis["birdeye"].get_token_volume(token_address)
            
            response_time = (time.time() - start_time) * 1000
            result["response_time_ms"] = response_time
            
            if token_overview or price_data or volume_data:
                result["success"] = True
                result["raw_data"] = {
                ...
                if volume_data:
                    data_points += len([k for k, v in volume_data.items() if v is not None])
    ...
    async def test_jupiter_api(self, token_address: str, symbol: str) -> Dict[str, Any]:
        ...
        try:
            # Test Jupiter price and volume endpoints
            price_data = await self.apis["jupiter"].get_token_price(token_address)
            volume_data = await self.apis["jupiter"].get_token_volume_24h(token_address)
            
            response_time = (time.time() - start_time) * 1000
            result["response_time_ms"] = response_time
            
            if price_data or volume_data:
                result["success"] = True
                result["raw_data"] = {
                    "price": price_data,
                    "volume": volume_data
                ...
                result["data_points"] = data_points
                result["data_quality"] = "excellent" if data_points > 10 else "good" if data_points > 5 else "basic"
    ...
    async def test_orca_api(self, token_address: str, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if pool_data:
                result["success"] = True
                result["raw_data"] = pool_data
                
                # Count data points
                data_points = len([k for k, v in pool_data.items() if v is not None]) if isinstance(pool_data, dict) else len(pool_data) if isinstance(pool_data, list) else 0
                result["data_points"] = data_points
                result["data_quality"] = "excellent" if data_points > 5 else "good" if data_points > 2 else "basic"
    ...
    async def test_raydium_api(self, token_address: str, symbol: str) -> Dict[str, Any]:
        ...
        try:
            pool_data = await self.apis["raydium"].get_token_pools(token_address)
    ...
    async def test_dexscreener_api(self, token_address: str, symbol: str) -> Dict[str, Any]:
        """Test DexScreener API for a token"""
        start_time = time.time()
        result = {
    ...
    async def test_all_apis_for_token(self, symbol: str, token_address: str) -> Dict[str, Any]:
        """Test all APIs for a single token"""
        self.logger.info(f"🔍 Testing all APIs for {symbol} ({token_address})")
        ...
        for result in results:
            if isinstance(result, dict):
                ...
                self.logger.info(f"  {status} {api_name}: {result['response_time_ms']:.1f}ms, {result['data_points']} data points")
            else:
    ...
    async def run_comprehensive_test(self):
        """Run comprehensive API test for all tokens"""
        self.logger.info("🚀 Starting Comprehensive API Comparison Test")
        self.logger.info(f"Testing {len(self.test_tokens)} tokens across {len(self.apis)} APIs")
    ...
    def display_summary(self):
        ...
        print("\n" + "="*80)
        print("🏆 COMPREHENSIVE API COMPARISON TEST RESULTS")
        print("="*80)
        ...
        print(tabulate(perf_data, 
                      headers=["API", "Success Rate", "Avg Response", "Avg Data Points", "Successful", "Total Calls"],
                      tablefmt="grid"))
    ...
    def save_results(self):
        ...
        filename = f"scripts/results/comprehensive_api_comparison_test_{timestamp}.json"
        ...
        summary_filename = f"scripts/results/api_comparison_summary_{timestamp}.txt"
        with open(summary_filename, 'w') as f:
            f.write("COMPREHENSIVE API COMPARISON TEST SUMMARY\n")
            f.write("="*50 + "\n\n")
...
async def main():
    """Main function to run the comprehensive API comparison test"""
    try:
        tester = ComprehensiveAPIComparison()
