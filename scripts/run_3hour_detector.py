...
async def run_3hour_detector():
    ...
    # Initialize enhanced structured logger for the main function
    enhanced_logger = create_enhanced_logger("run-3hour-detector-main")
    
    # Start scan context for the entire 3-hour session
    session_scan_id = enhanced_logger.new_scan_context(
        strategy="3hour-progressive-detection",
        timeframe="3_hours_9_cycles"
    )
    
    enhanced_logger.info("🚀 Initializing 3-hour progressive detection system",
                        session_scan_id=session_scan_id,
                        total_duration_hours=3,
                        interval_minutes=20,
                        total_cycles=9,
                        system_features=["4-stage-progressive", "batch-optimization", "structured-logging"])
    ...
    with enhanced_logger.stage_context(DetectionStage.INITIALIZATION, 
        ...
        print(f"📊 Enhanced Logging: Structured JSON with full traceability")
        print("=" * 60)
        
        enhanced_logger.info("Session configuration initialized",
                           working_directory=os.getcwd(),
                           python_executable=sys.executable,
                           features=["4-stage-progressive", "cost-optimization", "structured-logging"])
    ...
    if debug_mode:
        if stage0_debug:
            ...
            # Set environment variables for stage0 debug that can be accessed by all components
            os.environ['STAGE0_DEBUG'] = 'true'
            os.environ['PUMP_FUN_DEBUG'] = 'true' 
            os.environ['DISCOVERY_VERBOSE'] = 'true'
            enhanced_logger.info("🐛 Debug mode activated", 
                               debug_type="stage0_verbose",
                               debug_features=["pump.fun_discovery", "verbose_logging"],
                               environment_vars=["STAGE0_DEBUG", "PUMP_FUN_DEBUG", "DISCOVERY_VERBOSE"])
        else:
            print("🐛 Debug mode enabled - verbose logging activated")
            # Set general debug environment variable
            os.environ['GENERAL_DEBUG'] = 'true'
            enhanced_logger.info("🐛 Debug mode activated", 
                               debug_type="general",
                               debug_features=["verbose_logging"],
                               environment_vars=["GENERAL_DEBUG"])
    ...
    if futuristic_dashboard or futuristic_compact:
        if STYLED_DASHBOARD_AVAILABLE:
            styled_dashboard = create_futuristic_dashboard(debug_mode=debug_mode)
    ...
    if (dashboard_mode or compact_dashboard) and DASHBOARD_AVAILABLE:
        dashboard = create_dashboard(debug_mode=debug_mode)
        if dashboard_mode:
    ...
    if web_dashboard_mode and WEB_DASHBOARD_AVAILABLE:
        import threading
        web_dashboard = VirtuosoWebDashboard(port=dashboard_port, debug_mode=debug_mode)
    ...
    # Initialize detector with comprehensive debug parameters
    try:
        print("🔧 Initializing optimized detector...")
        
        # Log detector initialization with enhanced structured logging
        enhanced_logger.info("Initializing EarlyGemDetector",
                           debug_mode=debug_mode,
                           stage0_debug=stage0_debug,
                           initialization_context="detector_setup")
        
        # Initialize detector with comprehensive debug mode
        detector = EarlyGemDetector(debug_mode=debug_mode, stage0_debug=stage0_debug)
        
        # Pass stage0_debug to detector if it supports it
        if hasattr(detector, 'stage0_debug'):
            detector.stage0_debug = stage0_debug
            enhanced_logger.info("Stage0 debug mode set on detector", stage0_debug=stage0_debug)
        elif stage0_debug:
            # Set debug flag on any pump.fun related services
            if hasattr(detector, 'pump_fun_monitor') and detector.pump_fun_monitor:
                if hasattr(detector.pump_fun_monitor, 'debug_mode'):
                    detector.pump_fun_monitor.debug_mode = True
                    enhanced_logger.info("Stage0 debug enabled on pump.fun monitor")
            
            # Set debug on token discovery service if available
            if hasattr(detector, 'token_discovery_service') and detector.token_discovery_service:
                if hasattr(detector.token_discovery_service, 'debug_mode'):
                    detector.token_discovery_service.debug_mode = True
                    enhanced_logger.info("Stage0 debug enabled on token discovery service")
            
            enhanced_logger.info("Stage0 debug mode applied to available services")
        ...
        # Apply debug configurations to API connectors and services
        if debug_mode or stage0_debug:
            enhanced_logger.info("Applying debug configurations to API connectors",
                               debug_mode=debug_mode,
                               stage0_debug=stage0_debug)
            
            # Apply debug to BirdEye API if available
            if hasattr(detector, 'birdeye_api') and detector.birdeye_api:
                if hasattr(detector.birdeye_api, 'debug_mode'):
                    detector.birdeye_api.debug_mode = debug_mode
                    enhanced_logger.info("Debug mode applied to BirdEye API")
            
            # Apply debug to Moralis connector if available
            if hasattr(detector, 'moralis_connector') and detector.moralis_connector:
                if hasattr(detector.moralis_connector, 'debug_mode'):
                    detector.moralis_connector.debug_mode = debug_mode
                    enhanced_logger.info("Debug mode applied to Moralis connector")
            
            # Apply debug to batch API manager if available
            if hasattr(detector, 'batch_api_manager') and detector.batch_api_manager:
                if hasattr(detector.batch_api_manager, 'debug_mode'):
                    detector.batch_api_manager.debug_mode = debug_mode
                    enhanced_logger.info("Debug mode applied to batch API manager")
            
            # Apply stage0 specific debug to discovery services
            if stage0_debug:
                # Apply to any early token detection services
                if hasattr(detector, 'early_token_detection') and detector.early_token_detection:
                    if hasattr(detector.early_token_detection, 'debug_verbose'):
                        detector.early_token_detection.debug_verbose = True
                        enhanced_logger.info("Verbose debug mode applied to early token detection")
                
                enhanced_logger.info("Stage0 verbose debug configurations applied")
    ...
    try:
        ...
        while time.time() < session_end and cycle_number <= TOTAL_CYCLES:
            ...
            try:
                ...
                if debug_mode:
                    print(f"🐛 Debug mode: {'Stage0 Verbose' if stage0_debug else 'General'}")
                print(f"{'='*40}")
                
                # Log cycle start with enhanced structured logging
                with enhanced_logger.stage_context(DetectionStage.STAGE_0_DISCOVERY,
                                                 cycle_number=cycle_number,
                                                 total_cycles=TOTAL_CYCLES,
                                                 debug_mode=debug_mode,
                                                 stage0_debug=stage0_debug):
                    
                    enhanced_logger.info("Starting detection cycle",
                                       cycle_number=cycle_number,
                                       debug_configuration={
                                           "general_debug": debug_mode,
                                           "stage0_verbose": stage0_debug,
                                           "dashboard_enabled": web_dashboard_mode,
                                           "futuristic_dashboard": futuristic_dashboard
                                       })
        ...
        # Log comprehensive session summary with enhanced structured logging
        enhanced_logger.log_performance_summary(
            session_complete=True,
            total_cycles_completed=successful_cycles + failed_cycles,
            successful_cycles=successful_cycles,
            failed_cycles=failed_cycles,
            success_rate=round((successful_cycles/(successful_cycles+failed_cycles)*100), 2) if (successful_cycles + failed_cycles) > 0 else 0,
            total_tokens_found=total_tokens_found,
            total_alerts_sent=total_alerts_sent,
            session_duration_hours=round((time.time() - session_start) / 3600, 2)
        )
        
        enhanced_logger.info("🎉 3-hour progressive detection session completed",
                           session_scan_id=session_scan_id,
                           final_success=successful_cycles > 0,
                           session_summary={
                               "cycles_completed": successful_cycles + failed_cycles,
                               "success_rate": f"{(successful_cycles/(successful_cycles+failed_cycles)*100):.1f}%" if (successful_cycles + failed_cycles) > 0 else "0%",
                               "tokens_found": total_tokens_found,
...
if __name__ == '__main__':
    # Initialize main execution logger
    main_logger = create_enhanced_logger("run-3hour-detector-main-execution")
    main_scan_id = main_logger.new_scan_context(
        strategy="3hour-detector-execution",
        timeframe="full_session"
    )
    
    main_logger.info("🚀 Starting enhanced 3-hour detector with full structlog integration",
                    main_scan_id=main_scan_id,
                    logging_system="structlog_enhanced",
                    features=["structured_logging", "performance_tracking", "api_monitoring"])
    
    try:
        with main_logger.stage_context(DetectionStage.INITIALIZATION, 
                                     operation="main_execution"):
            success = asyncio.run(run_3hour_detector())
            
        if success:
            print("\n🎉 3-hour 4-stage progressive session completed successfully!")
            main_logger.info("🎉 Enhanced 3-hour detector completed successfully with structlog!",
                           main_scan_id=main_scan_id,
                           final_status="success")
        else:
            print("\n💥 3-hour 4-stage progressive session failed!")
            main_logger.error("💥 Enhanced 3-hour detector failed!",
                            main_scan_id=main_scan_id,
                            final_status="failed")
            sys.exit(1)
    except KeyboardInterrupt:
        print("\n⚠️ Session interrupted by user")
        main_logger.warning("⚠️ Enhanced detector interrupted by user",
                          main_scan_id=main_scan_id,
                          interruption_type="keyboard_interrupt")
        sys.exit(0)
    except Exception as e:
        print(f"\n❌ Unexpected error: {e}")
        main_logger.error("❌ Unexpected error in enhanced detector",
                        main_scan_id=main_scan_id,
                        error=str(e),
