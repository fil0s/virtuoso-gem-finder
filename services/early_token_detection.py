...
class EarlyTokenDetector:
    ...
    def _calculate_enhanced_age_score_and_bonus(self, creation_time: float, current_time: float) -> Tuple[float, float]:
        """
        Calculate enhanced age score with 8-tier system and bonus multipliers.
        
        Returns:
            Tuple[float, float]: (age_score, bonus_multiplier)
        """
        ...
        if age_hours <= 0.5:  # ≤30 minutes
            return 120, 0.20  # Ultra-new: 120 points + 20% total score bonus
        elif age_hours <= 2:  # 30min-2h
            return 110, 0.10  # Extremely new: 110 points + 10% total score bonus
        elif age_hours <= 6:  # 2-6h
            return 100, 0.0   # Very new: 100 points
        elif age_hours <= 24:  # 6-24h
            return 85, 0.0    # New: 85 points
        elif age_hours <= 72:  # 1-3 days
            return 65, 0.0    # Recent: 65 points
        elif age_hours <= 168:  # 3-7 days
            return 45, 0.0    # Moderate: 45 points
        elif age_hours <= 720:  # 7-30 days
            return 25, 0.0    # Established: 25 points
        else:  # >30 days
            return 10, 0.0    # Mature: 10 points
    ...
    async def _discover_and_analyze(self, max_tokens: int = 50, scan_id: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # STAGE 0: Check for pump.fun priority tokens (NEW!)
            stage0_tokens = await self._get_pump_fun_priority_tokens()
            if stage0_tokens:
                self.logger.info(f"🚀 Processing {len(stage0_tokens)} pump.fun Stage 0 priority tokens")
    ...
    async def _calculate_quick_scores(self, tokens: List[Dict], basic_metrics: Dict[str, Dict]) -> List[Tuple[Dict, float]]:
        ...
        for token in tokens:
            ...
            # Enhanced age score (25%) - 8-tier system with bonus multipliers
            creation_time = token.get('creation_time', current_time)
            age_score, age_bonus_multiplier = self._calculate_enhanced_age_score_and_bonus(creation_time, current_time)
            ...
            # Apply age bonus multiplier to final score (for ultra-new and extremely new tokens)
    ...
    async def _calculate_comprehensive_score(self, token: Dict, full_data: Dict, basic_metrics: Dict, security_data: Dict) -> float:
        ...
        try:
            ...
            # 2. Enhanced Age Score (25% weight) - 8-tier system with bonus multipliers
            creation_time = basic_metrics.get(token_address, {}).get('creation_time')
            current_time = time.time()
            if creation_time:
                age_score, age_bonus_multiplier = self._calculate_enhanced_age_score_and_bonus(creation_time, current_time)
                scores['age'] = age_score * (self.scoring_weights['age'] / 0.25)  # Scale to current weight
                age_hours = (current_time - creation_time) / 3600
                
                bonus_text = f" + {((age_bonus_multiplier - 1) * 100):.0f}% bonus" if age_bonus_multiplier > 1.0 else ""
                self.logger.debug(f"[SCORING] {token_symbol} - Age: {age_hours:.1f}h -> Score: {scores['age']:.1f}/25{bonus_text}")
            else:
                scores['age'] = 10 * (self.scoring_weights['age'] / 0.25)  # Scale default score
                age_bonus_multiplier = 1.0
                self.logger.debug(f"[SCORING] {token_symbol} - Age: Unknown -> Default Score: {scores['age']:.1f}/25")
            ...
            # Apply age bonus multiplier to final score (for ultra-new and extremely new tokens)
            final_score *= age_bonus_multiplier
            ...
            self.logger.info(f"  🏦 Liquidity (28%):     {scores['liquidity']:2.0f}/28")
            self.logger.info(f"  ⏰ Age (25%):          {age_score:2.0f}/25") 
            self.logger.info(f"  🚀 Price Gains (18%):  {price_score:2.0f}/18")
            self.logger.info(f"  💰 Volume (14%):       {volume_score:2.0f}/14")
            ...
            if age_bonus_multiplier > 1.0:
                self.logger.info(f"  ⚡ AGE BONUS MULTIPLIER: x{age_bonus_multiplier:.2f}")
    ...
    # NEW: Pump.fun Stage 0 Integration Methods
    async def _get_pump_fun_priority_tokens(self) -> List[Dict[str, Any]]:
        """Get pump.fun Stage 0 priority tokens from integration layer"""
        try:
            # Check if pump.fun integration is available
            if hasattr(self, 'pump_fun_integration') and self.pump_fun_integration:
                ...
                if priority_queue:
                    self.logger.info(f"🔥 Found {len(priority_queue)} Stage 0 pump.fun tokens in priority queue")
                    return await self._process_stage0_priority_tokens(priority_queue)
            
            return []
            
        except Exception as e:
            self.logger.error(f"❌ Error getting pump.fun priority tokens: {e}")
            return []
    
    async def _process_stage0_priority_tokens(self, priority_tokens: List[Dict] = None) -> List[Dict[str, Any]]:
        """Process pump.fun Stage 0 tokens with highest priority"""
        try:
            if not priority_tokens:
                # Get from integration if not provided
                if hasattr(self, 'pump_fun_integration') and self.pump_fun_integration:
                    priority_tokens = self.pump_fun_integration.get_stage0_priority_queue()
                else:
                    return []
            
            stage0_results = []
            for token in priority_tokens:
                # Normalize token address field for consistency
                token_address = token.get('token_address') or token.get('address', 'UNKNOWN')
                
                # Apply maximum scoring bonuses for Stage 0
                enhanced_token = {
                    **token,
                    'address': token_address,  # Ensure standard 'address' field
                    'stage0_bonus': 25,           # Maximum early detection bonus
                    'pump_fun_bonus': 20,         # Pump.fun specific bonus  
                ...
                if 'token_address' in token:
                    enhanced_token['token_address'] = token_address
                
                stage0_results.append(enhanced_token)
                self.logger.info(f"🚀 Stage 0 enhanced: {token_address[:8]}... with {enhanced_token.get('stage0_bonus', 0)} bonus points")
            
            return stage0_results
            
        except Exception as e:
            self.logger.error(f"❌ Error processing Stage 0 tokens: {e}")
            return []
    
    def initialize_pump_fun_integration(self, integration_instance):
        """Initialize pump.fun integration for Stage 0 detection"""
        try:
            self.pump_fun_integration = integration_instance
            self.logger.info("🚀 Pump.fun Stage 0 integration connected to early token detector")
        except Exception as e:
            self.logger.error(f"❌ Failed to initialize pump.fun integration: {e}")
