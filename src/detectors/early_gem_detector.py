...
def load_early_gem_scorer():
    ...
    scorer_path = os.path.join(os.path.dirname(script_dir), 'scoring', 'early_gem_focused_scoring.py')
...
class EarlyGemDetector:
    """
    🚀 EARLY GEM DETECTOR - 4-Stage Progressive Analysis + DexScreener Cost Optimization
    
    💰 NEW: ENHANCED COST OPTIMIZATION (60-80% Birdeye API Reduction)
    ================================================================
    
    📊 DISCOVERY OPTIMIZATION:
    ✅ PRIMARY: DexScreener trending discovery (FREE)
      • Token profiles with rich metadata + social signals  
      • Boosted tokens with marketing intelligence
      • Narrative-based search (AI, gaming, meme themes)
    ⚠️ SECONDARY: Birdeye trending (PAID - fallback only)
    
    📈 DATA ENHANCEMENT OPTIMIZATION:  
    ✅ PRIMARY: DexScreener comprehensive data extraction (FREE)
      • Volume, price, trading data (all timeframes)
      • Batch processing (30x API efficiency)
    ⚠️ SECONDARY: Selective Birdeye enhancement (PAID - minimal usage)
    
    🎯 4-Stage Architecture (Enhanced):
    ✅ Stage 1: Smart Discovery Triage (FREE - DexScreener-first, 70%+ reduction)
    ✅ Stage 2: Enhanced Analysis (MEDIUM - optimized data sources, 60%+ reduction)  
    ✅ Stage 3: Market Validation (MEDIUM - reduced API dependencies, 50-60% reduction)
    ✅ Stage 4: OHLCV Final Analysis (EXPENSIVE - selective Birdeye usage only)
    ...
    def __init__(self, config_path: str = "config/config.yaml", debug_mode: bool = False, stage0_debug: bool = False):
        """Initialize Early Gem Detector with 4-stage progressive analysis system"""
        
        self.debug_mode = debug_mode
        self.stage0_debug = stage0_debug or os.getenv('STAGE0_DEBUG', '').lower() == 'true'
        ...
        # Create scan context for the detector session
        self.scan_id = self.enhanced_logger.new_scan_context(
            strategy="4-stage-progressive-detection",
            timeframe="early_gem_detection"
        )
        
        # Setup traditional logging for backward compatibility
        self.logger = self._setup_logging()
        
        with self.enhanced_logger.stage_context(DetectionStage.INITIALIZATION, 
                                               operation="detector_initialization",
                                               debug_mode=self.debug_mode,
                                               stage0_debug=self.stage0_debug):
            self.enhanced_logger.info("🚀 Early Gem Detector initializing", 
                                    debug_mode=self.debug_mode,
                                    stage0_debug=self.stage0_debug,
        ...
        # Log initialization completion with structured logging
        with self.enhanced_logger.stage_context(DetectionStage.INITIALIZATION, 
                                               operation="initialization_complete"):
            self.enhanced_logger.info("✅ Early Gem Detector initialized successfully",
                                    high_conviction_threshold=self.high_conviction_threshold,
                                    moralis_active=bool(self.moralis_connector),
                                    telegram_enabled=bool(self.telegram_alerter),
                                    sol_bonding_active=bool(self.sol_bonding_detector),
                                    debug_features={
                                        "debug_mode": self.debug_mode,
                                        "stage0_debug": self.stage0_debug,
                                        "early_gem_focused": True,
        ...
        if self.stage0_debug:
            self.enhanced_logger.debug("🐛 Stage0 debug mode activated",
                                     debug_features=["pump_fun_discovery", "verbose_token_analysis", "api_call_tracing"])
    ...
    async def discover_early_tokens(self) -> List[Dict[str, Any]]:
        ...
        # Platform 1: DexScreener trending (FREE - primary discovery)
        discovery_tasks.append(self._discover_dexscreener_trending())
        
        # Platform 2: Moralis graduated tokens  
        discovery_tasks.append(self._fetch_moralis_graduated_tokens())
        
        # Platform 3: Moralis bonding tokens (Pre-graduation detection)
        discovery_tasks.append(self._fetch_moralis_bonding_tokens())
        
        # Platform 4: Raydium v3 enhanced early gem detection
        discovery_tasks.append(self._fetch_raydium_v3_pools())
        
        # Platform 5: Birdeye trending (PAID - comprehensive discovery)
        ...
        try:
            ...
            # Process results  
            platform_names = ["DexScreener", "Moralis Graduated", "Moralis Bonding", "Raydium v3 Enhanced", "Birdeye Trending"]
            dexscreener_count = 0
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    ...
                    if i == 0:  # DexScreener result
                        dexscreener_count = result_count
                    self.logger.info(f"   📊 {platform_names[i]}: {result_count} tokens")
            ...
            try:
                dexscreener_tokens = await self._discover_dexscreener_trending()
                all_candidates.extend(dexscreener_tokens)
                dexscreener_count = len(dexscreener_tokens)
                self.logger.info(f"   📊 DexScreener: {dexscreener_count} tokens")
            except Exception as e:
                self.logger.error(f"❌ DexScreener discovery failed: {e}")
            
            # Other platforms
            for task_func, name in [(self._fetch_moralis_graduated_tokens, "Moralis Graduated"),
                                  (self._fetch_moralis_bonding_tokens, "Moralis Bonding"),
                                  (self._fetch_raydium_v3_pools, "Raydium v3 Enhanced")]:
                try:
            ...
            # Always include Birdeye trending in sequential mode too
            self.logger.info(f"🐦 Sequential: Adding Birdeye trending tokens...")
            try:
                birdeye_tokens = await self._fetch_birdeye_trending_tokens()
                all_candidates.extend(birdeye_tokens)
                self.logger.info(f"   📊 Birdeye trending: {len(birdeye_tokens)} tokens")
            except Exception as e:
                self.logger.error(f"❌ Birdeye trending failed: {e}")
    ...
    async def _discover_dexscreener_trending(self) -> List[Dict[str, Any]]:
        """
        NEW: DexScreener-based trending discovery - FREE alternative to Birdeye trending
        Uses token profiles and boosted tokens to identify trending opportunities
        """
        candidates = []
        
        try:
            self.logger.info("🚀 Starting DexScreener trending discovery (FREE)")
            
            # Method 1: Get token profiles (rich metadata)
            profiles = await self._get_dexscreener_token_profiles()
            for profile in profiles:
                if profile.get('address'):
                    candidate = {
                        'address': profile['address'],
                        'symbol': profile.get('symbol', f"PROFILE_{profile['address'][:8]}"),
                        'name': profile.get('name', 'DexScreener Profile Token'),
                        'source': 'dexscreener_profiles',
                        'estimated_age_minutes': 30,  # Recent profiles are fresh
                        'platforms': ['dexscreener'],
                        'social_score': profile.get('social_score', 0.5),
                        'narrative_strength': profile.get('narrative_strength', 0.3),
                        'dexscreener_featured': True,
                        'discovery_method': 'token_profiles'
                    }
                    candidates.append(candidate)
            
            # Method 2: Get boosted tokens (marketing intelligence)
            boost_analysis = await self._get_dexscreener_boosted_analysis()
            
            # Process latest boosted tokens
            for boost in boost_analysis.get('latest_boosted', []):
                if boost.get('address'):
                    candidate = {
                        'address': boost['address'],
                        'symbol': f"BOOST_{boost['address'][:8]}",
                        'name': 'DexScreener Boosted Token',
                        'source': 'dexscreener_boosted',
                        'estimated_age_minutes': 60,
                        'platforms': ['dexscreener'],
                        'boost_amount': boost.get('total_amount', 0),
                        'marketing_intensity': boost.get('marketing_intensity', 'low'),
                        'consumption_rate': boost.get('consumption_rate', 0.0),
                        'discovery_method': 'boosted_tokens'
                    }
                    candidates.append(candidate)
            
            # Method 3: Narrative-based discovery
            popular_narratives = ["AI", "agent", "gaming", "DeFi", "meme", "pump"]
            narrative_results = await self._discover_dexscreener_narrative_tokens(popular_narratives)
            
            for narrative, tokens in narrative_results.items():
                for token in tokens[:3]:  # Limit per narrative to avoid spam
                    if token.get('address'):
                        candidate = {
                            'address': token['address'],
                            'symbol': token.get('symbol', f"NARRATIVE_{token['address'][:8]}"),
                            'name': token.get('name', f"{narrative.title()} Token"),
                            'source': f'dexscreener_narrative_{narrative}',
                            'estimated_age_minutes': 120,
                            'platforms': ['dexscreener'],
                            'narrative': narrative,
                            'narrative_relevance': token.get('narrative_relevance', 0.5),
                            'volume_24h': token.get('volume_24h', 0),
                            'liquidity': token.get('liquidity', 0),
                            'discovery_method': 'narrative_search'
                        }
                        candidates.append(candidate)
            
            self.logger.info(f"✅ DexScreener trending discovery completed: {len(candidates)} candidates found")
            ...
            for method, count in discovery_methods.items():
                self.logger.debug(f"   📊 {method}: {count} candidates")
            
        except Exception as e:
            self.logger.error(f"Error in DexScreener trending discovery: {e}")
            
        return candidates

    async def _discover_dexscreener_narrative_tokens(self, narratives: List[str]) -> Dict[str, List[Dict[str, Any]]]:
        """
        Discover tokens by narrative themes using DexScreener search API
        This provides narrative-based discovery without Birdeye costs
        """
        narrative_results = {}
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                for narrative in narratives:
                    try:
                        url = f'https://api.dexscreener.com/latest/dex/search?q={narrative}'
                                ...
                                if 'pairs' in search_data:
                                    for pair in search_data['pairs']:
                                        if pair.get('chainId') == 'solana':
                                            ...
                                            if volume_24h >= 5000 and liquidity >= 20000:  # Quality thresholds
                                                token_data = {
                                                    ...
                                                    'fdv': pair.get('fdv', 0),
                                                    'narrative_relevance': self._calculate_narrative_relevance_simple(base_token, narrative)
                                                }
                                                
                                                solana_tokens.append(token_data)
                                
                                # Sort by relevance and volume
                                solana_tokens.sort(key=lambda x: (x['narrative_relevance'], x['volume_24h']), reverse=True)
                                narrative_results[narrative] = solana_tokens[:10]  # Top 10 per narrative
                                
                                self.logger.debug(f"🎯 DexScreener narrative '{narrative}': {len(solana_tokens)} qualified tokens")
                                
                    except Exception as e:
                        self.logger.warning(f"Error searching DexScreener for narrative '{narrative}': {e}")
            ...
            self.logger.error(f"Error in DexScreener narrative discovery: {e}")
        
        return narrative_results

    async def _search_dexscreener_tokens(self, query: str, limit: int = 20) -> List[Dict[str, Any]]:
        """
        Search for tokens by symbol/name using DexScreener search API
        FREE alternative to expensive Birdeye token search operations
        Rate limit: 300 RPM
        """
        try:
            import aiohttp
                        ...
                        if 'pairs' in search_data:
                            for pair in search_data['pairs']:
                                if pair.get('chainId') == 'solana':
                                    ...
                                    # Extract comprehensive data from search results
                                    ...
                                    token_data = {
                                        ...
                                        'search_relevance': self._calculate_search_relevance(base_token, query),
                                        'data_source': 'dexscreener_search',
                                        'source': 'dexscreener_search',
                                        'platforms': ['dexscreener']
                                    }
                                    
                                    # Quality filter for search results
                                    if (token_data['volume_24h'] >= 1000 and 
                                        token_data['liquidity_usd'] >= 5000 and 
                                        token_data['address']):
                                        solana_tokens.append(token_data)
                        
                        # Sort by search relevance and volume
                        solana_tokens.sort(key=lambda x: (x['search_relevance'], x['volume_24h']), reverse=True)
                        
                        self.logger.info(f"✅ DexScreener search '{query}': Found {len(solana_tokens)} qualified tokens")
                        return solana_tokens[:limit]
                    
                    else:
                        self.logger.debug(f"DexScreener search failed for '{query}': Status {response.status}")
                        return []
                        
        except Exception as e:
            self.logger.error(f"Error in DexScreener token search for '{query}': {e}")
            return []

    def _calculate_search_relevance(self, token_info: Dict[str, Any], query: str) -> float:
        ...
        elif any(word in symbol for word in query_lower.split()):
            relevance += 0.4
        elif any(word in name for word in query_lower.split()):
            relevance += 0.3
        else:
            relevance += 0.2  # Base score for any match
        
        return min(1.0, relevance)

    async def _get_dexscreener_pair_data(self, chain_id: str, pair_address: str) -> Dict[str, Any]:
        """
        Get detailed pair information from DexScreener
        Provides liquidity depth, pair creation time, and advanced trading metrics
        Rate limit: 300 RPM
        """
        try:
            import aiohttp
                ...
                async with session.get(url) as response:
                    if response.status == 200:
                        ...
                        if 'pairs' in data and data['pairs']:
                            ...
                            # Extract comprehensive pair data
                            volume = pair.get('volume', {})
                            txns = pair.get('txns', {})
                            liquidity = pair.get('liquidity', {})
                            price_change = pair.get('priceChange', {})
                            ...
                            pair_data = {
                                ...
                                # Volume metrics (comprehensive timeframes)
                                ...
                                # Transaction data with granular timeframes
                                ...
                                'buy_sell_ratio_5m': self._calculate_buy_sell_ratio(txns.get('m5', {})),
                                'buy_sell_ratio_1h': self._calculate_buy_sell_ratio(txns.get('h1', {})),
                                'buy_sell_ratio_24h': self._calculate_buy_sell_ratio(txns.get('h24', {})),
                                
                                # Momentum indicators
                                'volume_momentum': self._calculate_volume_momentum(volume),
                                'price_momentum': self._calculate_price_momentum(price_change),
                                'trading_intensity': self._calculate_trading_intensity(txns, volume),
                                
                                'data_source': 'dexscreener_pair'
                            }
                            
                            self.logger.info(f"✅ DexScreener pair data for {pair_address}: {pair_data['base_token']['symbol']}/{pair_data['quote_token']['symbol']}")
                            return pair_data
                        
                        else:
                            self.logger.debug(f"DexScreener: No pair data found for {pair_address}")
                            return {}
                    
                    else:
                        self.logger.debug(f"DexScreener pair request failed: Status {response.status}")
                        return {}
                        
        except Exception as e:
            self.logger.error(f"Error fetching DexScreener pair data for {pair_address}: {e}")
            return {}

    def _calculate_buy_sell_ratio(self, txn_data: Dict[str, Any]) -> float:
        """Calculate buy/sell ratio for trading sentiment analysis"""
        buys = txn_data.get('buys', 0)
        sells = txn_data.get('sells', 0)
        
        if sells == 0:
            return 10.0 if buys > 0 else 1.0  # Cap at 10 if no sells
        
        return min(10.0, buys / sells)  # Cap at 10 for extreme ratios

    def _calculate_volume_momentum(self, volume_data: Dict[str, Any]) -> float:
        """Calculate volume momentum across timeframes"""
        vol_5m = volume_data.get('m5', 0)
        vol_1h = volume_data.get('h1', 0) 
        vol_6h = volume_data.get('h6', 0)
        vol_24h = volume_data.get('h24', 0)
        
        # Calculate momentum by comparing shorter to longer timeframes
        momentum_score = 0.0
        
        # 5m vs 1h (most recent activity)
        if vol_1h > 0 and vol_5m > 0:
            momentum_score += min(2.0, (vol_5m * 12) / vol_1h)  # 5m * 12 = hourly projection
        
        # 1h vs 6h  
        if vol_6h > 0 and vol_1h > 0:
            momentum_score += min(2.0, (vol_1h * 6) / vol_6h)
        
        # 6h vs 24h
        if vol_24h > 0 and vol_6h > 0:
            momentum_score += min(2.0, (vol_6h * 4) / vol_24h)
        
        return min(3.0, momentum_score)  # Cap at 3.0

    def _calculate_price_momentum(self, price_change_data: Dict[str, Any]) -> float:
        """Calculate price momentum score"""
        pc_5m = abs(price_change_data.get('m5', 0))
        pc_1h = abs(price_change_data.get('h1', 0))
        pc_6h = abs(price_change_data.get('h6', 0))
        pc_24h = abs(price_change_data.get('h24', 0))
        
        # Weight recent price movements higher
        momentum = (pc_5m * 4 + pc_1h * 3 + pc_6h * 2 + pc_24h * 1) / 10
        
        return min(50.0, momentum)  # Cap at 50% for extreme movements

    def _calculate_trading_intensity(self, txns_data: Dict[str, Any], volume_data: Dict[str, Any]) -> float:
        """Calculate trading intensity (transactions per volume)"""
        txns_24h = txns_data.get('h24', {})
        ...
        # Transactions per $1000 volume
        intensity = (total_txns / vol_24h) * 1000
        
        return min(10.0, intensity)  # Cap at 10 for normalization

    def _calculate_narrative_relevance_simple(self, token_info: Dict[str, Any], narrative: str) -> float:
        """Simple narrative relevance calculation"""
        ...
        # Direct matches
        if narrative_lower in symbol:
            relevance += 0.5
        elif narrative_lower in name:
            relevance += 0.3
        
        # Partial matches
        if any(word in symbol for word in narrative_lower.split()):
            relevance += 0.2
    ...
    def _init_launchlab_integration(self):
        """Initialize Pump.fun Bonding Curve Detector (proper implementation)"""
        try:
            from services.pump_fun_bonding_curve_detector import PumpFunBondingCurveDetector
            
            # Use the proper pump.fun client and moralis API for real bonding curve detection
            detector = PumpFunBondingCurveDetector(
                pump_fun_client=self.pump_fun_api,
                moralis_api=self.moralis_api
            )
            
            self.logger.info("🎯 Pump.fun Bonding Curve Detector initialized for early detection")
            self.logger.info("   🔥 REAL bonding curve detection (not DEX pairs)")
            self.logger.info("   📊 Uses Pump.fun APIs and Moralis bonding endpoints")
            self.logger.info("   🎯 85 SOL graduation threshold for bonding curve → AMM migration")
            self.logger.info("   ⚡ Performance: ~3-5 seconds for 20 tokens with caching")
            
            return detector
        except ImportError:
            self.logger.warning("⚠️ Pump.fun Bonding Curve Detector not available - bonding detection disabled")
            return None
        except Exception as e:
            self.logger.warning(f"⚠️ Pump.fun Bonding Curve Detector failed to initialize: {e}")
            return None
    ...
    async def run_detection_cycle(self) -> Dict[str, Any]:
        ...
        # Create cycle-specific context with enhanced structured logging
        cycle_scan_id = self.enhanced_logger.new_scan_context(
            strategy="4-stage-detection-cycle",
            timeframe="single_cycle"
        )
        
        with self.enhanced_logger.stage_context(DetectionStage.STAGE_0_DISCOVERY, 
                                               operation="detection_cycle_start",
                                               cycle_scan_id=cycle_scan_id):
            self.enhanced_logger.info("🚀 Starting Early Gem Detection Cycle",
                                    cycle_scan_id=cycle_scan_id,
                                    stages_planned=["discovery", "enrichment", "analysis", "validation"],
        ...
        try:
            ...
            with self.enhanced_logger.stage_context(DetectionStage.STAGE_0_DISCOVERY, 
                                                   operation="multi_platform_discovery",
                                                   cycle_scan_id=cycle_scan_id):
                self.enhanced_logger.info("🔍 Step 1: Multi-platform token discovery",
                                        discovery_sources=["pump_fun", "birdeye", "moralis"],
                                        stage0_debug_active=self.stage0_debug)
            ...
            with self.enhanced_logger.stage_context(DetectionStage.ANALYSIS, 
                                                   operation="token_enrichment",
                                                   cycle_scan_id=cycle_scan_id):
                self.enhanced_logger.info("🔥 Step 2: Enriching graduated tokens with market data",
                                        candidate_count=len(all_candidates))
                
                self.logger.info("🔥 Step 2: Enriching graduated tokens with market data")
                enrichment_start = time.time()
                
                with self.enhanced_logger.api_call_context(APICallType.BATCH_METADATA, 
                                                          "token_enrichment"):
                    enriched_candidates = await self._enrich_graduated_tokens(all_candidates)
                
                enrichment_time = time.time() - enrichment_start
                
                self.enhanced_logger.info("Token enrichment completed",
                                         enrichment_duration=enrichment_time,
            ...
            with self.enhanced_logger.stage_context(DetectionStage.ANALYSIS, 
                                                   ...
                                                   cycle_scan_id=cycle_scan_id):
                self.enhanced_logger.info("🎯 Step 2.5: Applying tiered resource allocation",
                                        optimization_strategy="4-stage-progressive",
                                        cost_reduction_target="60-70%")
...
#{symbol.replace(' ', '')}Gem #EarlyDetection #PumpFun"""
    ...
    def _init_apis(self):
        ...
        try:
            ...
            # Initialize RaydiumConnector for enhanced early gem detection
            try:
                self.raydium_connector = RaydiumConnector(
                    enhanced_cache=self.cache_manager,
                    api_tracking_enabled=True,
                    rate_limiter=self.rate_limiter
                )
                self.logger.info("⚡ Raydium v3 Connector initialized for early gem detection")
                self.logger.info("   📊 Enhanced strategy: Multi-page scanning with early gem prioritization")
                self.logger.info("   🎯 Target: TVL $1k-$500k, Volume/TVL ratio > 0.1")
            except Exception as e:
                self.raydium_connector = None
                self.logger.warning(f"⚠️ Raydium Connector failed to initialize: {e}")
    ...
    async def _fetch_sol_bonding_tokens(self) -> List[Dict[str, Any]]:
        """Fetch tokens from Pump.fun Bonding Curve Detector for enhanced analysis"""
        try:
            if not self.sol_bonding_detector:
                self.logger.debug("Pump.fun Bonding Curve Detector not available")
                return []
            
            self.logger.debug("🎯 Fetching real bonding curve candidates...")
            
            # Get REAL bonding curve candidates (not DEX pairs)
            sol_bonding_candidates = await self.sol_bonding_detector.get_bonding_curve_candidates(limit=20)
            ...
            if sol_bonding_candidates:
                ...
                for candidate in sol_bonding_candidates:
                    try:
                        # Convert Pump.fun bonding curve detector format to our standard format
                        standardized_candidate = {
                            'symbol': candidate.get('symbol', 'Unknown'),
                            'name': candidate.get('name', 'Pump.fun Bonding Token'),
                            ...
                            'graduation_threshold': candidate.get('graduation_threshold_usd', 69000),
                            'sol_in_bonding_curve': candidate.get('estimated_sol_raised', 0),
                            'price_change_5m': candidate.get('price_change_5m', 0),
                            'velocity_usd_per_hour': candidate.get('velocity_usd_per_hour', 0),
                            'source': 'pump_fun_bonding_detector',
                            'platforms': ['pump_fun'],
                            'pump_fun_stage': candidate.get('bonding_curve_stage', 'bonding'),
                            ...
                            'is_bonding_curve_token': candidate.get('is_bonding_curve_token', True),
                            'raw_bonding_curve_data': candidate
                        }
                        ...
                        self.logger.debug(f"   🔥 Bonding Curve: {standardized_candidate['symbol']} ({progress:.1f}% to graduation)")
                        
                    except Exception as e:
                        self.logger.debug(f"   ❌ Error processing SOL bonding candidate: {e}")
                        continue
            
            self.logger.info(f"🔥 Pump.fun Bonding Curve Detector found {len(candidates)} candidates")
            return candidates
            
        except Exception as e:
            self.logger.error(f"❌ Error fetching bonding curve tokens: {e}")
            return []

    async def _fetch_raydium_v3_pools(self) -> List[Dict[str, Any]]:
        """Fetch early gem candidates from Raydium v3 API with enhanced detection"""
        try:
            if not self.raydium_connector:
                self.logger.debug("Raydium v3 Connector not available")
                return []
            
            self.logger.debug("⚡ Fetching Raydium v3 early gem candidates...")
            
            # Use enhanced WSOL trending pairs with early gem detection
            raydium_pairs = await self.raydium_connector.get_wsol_trending_pairs(limit=25)
            
            candidates = []
            if raydium_pairs:
                self.logger.debug(f"📊 Processing {len(raydium_pairs)} Raydium v3 candidates...")
                
                # Separate early gems from regular pairs for logging
                early_gems = [p for p in raydium_pairs if p.get('is_early_gem_candidate', False)]
                regular_pairs = [p for p in raydium_pairs if not p.get('is_early_gem_candidate', False)]
                
                self.logger.info(f"💎 Found {len(early_gems)} early gem candidates, {len(regular_pairs)} regular pairs")
                
                for pair in raydium_pairs:
                    try:
                        # Convert Raydium v3 format to our standard format
                        standardized_candidate = {
                            'symbol': pair.get('symbol', 'Unknown'),
                            'name': f"Raydium Pool: {pair.get('pair_name', 'Unknown')}",
                            'address': pair.get('address', ''),
                            'market_cap': 0,  # Will be enriched later
                            'price': pair.get('price', 0),  
                            'volume_24h': pair.get('volume_24h', 0),
                            'liquidity': pair.get('tvl', pair.get('liquidity', 0)),
                            'tvl': pair.get('tvl', 0),
                            'volume_tvl_ratio': pair.get('volume_tvl_ratio', 0),
                            
                            # Early gem specific metrics
                            'is_early_gem_candidate': pair.get('is_early_gem_candidate', False),
                            'early_gem_score': pair.get('early_gem_score', 0),
                            'discovery_source': 'raydium_v3_enhanced',
                            'platform': 'raydium',
                            'platforms': ['raydium'],
                            'pool_address': pair.get('pool_address', ''),
                            'pair_type': pair.get('pair_type', ''),
                            'is_wsol_pair': pair.get('is_wsol_pair', True),
                            
                            # Quality indicators
                            'dex_presence': ['raydium'],
                            'liquidity_to_mcap_ratio': 0.0,  # Will be calculated after enrichment
                            'liquidity_quality_score': 8 if pair.get('is_early_gem_candidate') else 6,
                            
                            # Source tracking
                            'source': 'raydium_v3_pools',
                            'raw_data': pair
                        }
                        
                        candidates.append(standardized_candidate)
                        
                        # Enhanced logging for early gems
                        if pair.get('is_early_gem_candidate'):
                            self.logger.debug(f"   💎 Early Gem: {pair.get('symbol')} - TVL: ${pair.get('tvl', 0):,.0f}, Ratio: {pair.get('volume_tvl_ratio', 0):.1f}")
                        else:
                            self.logger.debug(f"   📊 Regular: {pair.get('symbol')} - Volume: ${pair.get('volume_24h', 0):,.0f}")
                        
                    except Exception as e:
                        self.logger.debug(f"   ❌ Error processing Raydium v3 candidate: {e}")
                        continue
            
            self.logger.info(f"⚡ Raydium v3 Enhanced Detector found {len(candidates)} candidates")
            return candidates
            
        except Exception as e:
            self.logger.error(f"❌ Error fetching Raydium v3 pools: {e}")
            return []
    ...
    async def _get_dexscreener_batch_data(self, token_addresses: List[str]) -> Dict[str, Dict[str, Any]]:
        """
        Batch fetch DexScreener data for multiple tokens - 30x more efficient than individual calls
        This is a key optimization to reduce API usage and maximize free DexScreener data extraction
        """
        batch_results = {}
        ...
        try:
            ...
            # DexScreener supports comma-separated addresses for batch queries
            batch_size = 30  # Optimized batch size using DexScreener API limit (33% fewer calls)
            
            for i in range(0, len(token_addresses), batch_size):
                batch_addresses = token_addresses[i:i + batch_size]
                addresses_str = ','.join(batch_addresses)
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(url) as response:
                        if response.status == 200:
                            ...
                            # Process batch response and extract data for each token
                            if 'pairs' in data:
                                for pair in data['pairs']:
                                    base_token = pair.get('baseToken', {})
                                    token_address = base_token.get('address', '')
                                    
                                    if token_address in batch_addresses:
                                        ...
                                        batch_results[token_address] = {
                                            ...
                                            # Transaction data
                                            'trades_24h': txns.get('h24', {}).get('buys', 0) + txns.get('h24', {}).get('sells', 0),
                                            'buys_24h': txns.get('h24', {}).get('buys', 0),
                                            'sells_24h': txns.get('h24', {}).get('sells', 0),
                                            
                                            'data_source': 'dexscreener_batch',
                                            'dex_id': pair.get('dexId', ''),
                                            'pair_address': pair.get('pairAddress', '')
                                        }
                                
                            self.logger.info(f"✅ DexScreener batch: Processed {len(batch_addresses)} tokens in single API call")
                            
                        await asyncio.sleep(0.3)  # Rate limiting between batches
                        
        except Exception as e:
            self.logger.error(f"Error in DexScreener batch fetch: {e}")
        
        return batch_results

    async def _get_dexscreener_token_profiles(self) -> List[Dict[str, Any]]:
        """
        Get latest token profiles from DexScreener - rich metadata and social signals
        This provides social validation data that can replace Birdeye trending in many cases
        """
        try:
            import aiohttp
                        ...
                        for profile in profiles:
                            if profile.get('chainId') == 'solana':
                                enhanced_profile = {
                                    ...
                                    'narrative_strength': min(1.0, len(profile.get('description', '')) / 200.0),
                                    'social_score': min(1.0, len(profile.get('links', [])) / 5.0),
                                }
                                
                                # Enhanced social scoring with comprehensive analysis
                                social_analysis = self._calculate_enhanced_social_score(profile)
                                enhanced_profile.update(social_analysis)
                                solana_profiles.append(enhanced_profile)
                        
                        self.logger.debug(f"✅ DexScreener profiles: Found {len(solana_profiles)} Solana token profiles")
                        return solana_profiles
                        
        except Exception as e:
            self.logger.error(f"Error fetching DexScreener token profiles: {e}")
        
        return []

    def _calculate_enhanced_social_score(self, profile: Dict[str, Any]) -> Dict[str, Any]:
        """
        Enhanced social scoring algorithm for DexScreener profiles
        Analyzes multiple factors for comprehensive social intelligence
        """
        description = profile.get('description', '')
        ...
        social_analysis = {
            'social_score': 0.0,
            'social_factors': {},
            'social_quality': 'unknown',
            'community_signals': {}
        }
        
        # 1. Link Analysis (40% of score)
        link_score = self._analyze_social_links(links)
        social_analysis['social_factors']['link_score'] = link_score
        
        # 2. Description Quality (25% of score)
        description_score = self._analyze_description_quality(description)
        social_analysis['social_factors']['description_score'] = description_score
        
        # 3. Profile Completeness (20% of score)
        completeness_score = self._analyze_profile_completeness(profile)
        social_analysis['social_factors']['completeness_score'] = completeness_score
        
        # 4. Community Indicators (15% of score)
        community_score = self._analyze_community_indicators(description, links)
        social_analysis['social_factors']['community_score'] = community_score
        ...
        social_analysis['social_score'] = min(1.0, total_score)
        
        # Determine quality tier
        if total_score >= 0.8:
            social_analysis['social_quality'] = 'excellent'
        elif total_score >= 0.6:
            social_analysis['social_quality'] = 'good'
        elif total_score >= 0.4:
            social_analysis['social_quality'] = 'moderate'
        else:
            social_analysis['social_quality'] = 'poor'
        
        return social_analysis

    def _analyze_social_links(self, links: List[Dict[str, Any]]) -> float:
        """Analyze social media links for quality and diversity"""
        if not links:
            return 0.0
        
        link_scores = {
            'twitter': 0.35,      # Highest value for Twitter presence
            'telegram': 0.25,     # Strong community indicator
            'discord': 0.20,      # Community engagement
            'website': 0.15,      # Professional presence
            'github': 0.15,       # Development transparency
            'medium': 0.10,       # Content/educational
            'youtube': 0.08,      # Content creation
            'instagram': 0.05,    # Visual marketing
            'linkedin': 0.05,     # Professional networking
            'reddit': 0.05        # Community discussion
        }
        ...
        for link in links:
            ...
            if link_type in link_scores:
                total_score += link_scores[link_type]
                link_types_found.add(link_type)
            
            # URL pattern matching for missing types
            elif 'twitter.com' in link_url or 'x.com' in link_url:
                total_score += link_scores['twitter']
                link_types_found.add('twitter')
            elif 't.me' in link_url:
                total_score += link_scores['telegram']
                link_types_found.add('telegram')
            elif 'discord' in link_url:
                total_score += link_scores['discord']
                link_types_found.add('discord')
            elif 'github.com' in link_url:
                total_score += link_scores['github']
                link_types_found.add('github')
        
        # Bonus for diversity (having multiple social platforms)
        diversity_bonus = min(0.2, len(link_types_found) * 0.05)
        
        # Penalty for too many links (spam indicator)
        spam_penalty = max(0.0, (len(links) - 8) * 0.02) if len(links) > 8 else 0.0
        
        final_score = total_score + diversity_bonus - spam_penalty
        return min(1.0, final_score)

    def _analyze_description_quality(self, description: str) -> float:
        """Analyze description quality and content depth"""
        if not description:
            return 0.0
        ...
        quality_indicators = [
            ('roadmap', 0.15), ('whitepaper', 0.15), ('tokenomics', 0.12),
            ('utility', 0.10), ('ecosystem', 0.10), ('partnership', 0.08),
            ('community', 0.08), ('governance', 0.08), ('staking', 0.06),
            ('defi', 0.05), ('nft', 0.05), ('gamefi', 0.05),
            ('audit', 0.12), ('kyc', 0.08), ('verified', 0.08)
        ]
        ...
        # Spam/scam indicators (negative scoring)
        spam_indicators = ['moon', '1000x', 'lambo', 'easy money', 'guaranteed', 'pump', 'wen']
        spam_count = sum(1 for spam in spam_indicators if spam in description_lower)
        spam_penalty = spam_count * 0.05
        
        # Grammar and structure (basic analysis)
        sentences = description.count('.') + description.count('!') + description.count('?')
        if sentences >= 3:
            quality_score += 0.05
        
        final_score = quality_score - spam_penalty
        return min(1.0, max(0.0, final_score))

    def _analyze_profile_completeness(self, profile: Dict[str, Any]) -> float:
        """Analyze profile completeness and professional presentation"""
        ...
        if profile.get('links') and len(profile['links']) > 0:
            completeness_score += 0.25
        
        return min(1.0, completeness_score)

    def _analyze_community_indicators(self, description: str, links: List[Dict[str, Any]]) -> float:
        """Analyze community engagement and growth indicators"""
        community_score = 0.0
        ...
        # Community-focused terms
        community_terms = [
            ('community-driven', 0.2), ('holder rewards', 0.15), ('dao', 0.15),
            ('airdrop', 0.1), ('rewards', 0.08), ('staking', 0.08),
            ('voting', 0.08), ('governance', 0.1), ('ambassador', 0.06)
        ]
        
        for term, score in community_terms:
            if term in description_lower:
                community_score += score
        
        # Social platform diversity bonus
        social_platforms = ['twitter', 'telegram', 'discord']
        social_count = sum(1 for link in links if link.get('type', '').lower() in social_platforms)
        if social_count >= 2:
            community_score += 0.1
        
        return min(1.0, community_score)

    async def _get_dexscreener_boosted_analysis(self) -> Dict[str, Any]:
        """
        Get comprehensive boosted tokens analysis for marketing intelligence
        This provides promotional momentum data that can supplement discovery
        """
        boosted_data = {
            'latest_boosted': [],
            'top_boosted': [],
                            ...
                            if response.status == 200:
                                ...
                                for boost in boosts:
                                    if boost.get('chainId') == 'solana':
                                        ...
                                        current_amount = boost.get('amount', 0)
                                        ...
                                        enhanced_boost = {
                                            ...
                                            'social_links': boost.get('links', []),
                                            'boost_url': boost.get('url', ''),
                                        }
                                        
                                        # Calculate consumption rate and intensity
                                        if total_amount > 0:
                                            enhanced_boost['consumption_rate'] = (total_amount - current_amount) / total_amount
                                        
                                        if total_amount >= 1000:
                                            enhanced_boost['marketing_intensity'] = 'very_high'
                                        elif total_amount >= 500:
                                            enhanced_boost['marketing_intensity'] = 'high'
                                        elif total_amount >= 100:
                                            enhanced_boost['marketing_intensity'] = 'medium'
                                        
                                        solana_boosts.append(enhanced_boost)
                                
                                if boost_type == 'latest':
                                    boosted_data['latest_boosted'] = solana_boosts
                                else:
                                    boosted_data['top_boosted'] = solana_boosts
                                
                    except Exception as e:
                        self.logger.warning(f"Error fetching {boost_type} boosted tokens: {e}")
                    
                    await asyncio.sleep(0.3)  # Rate limiting
            
            # Generate marketing insights summary
            all_boosts = boosted_data['latest_boosted'] + boosted_data['top_boosted']
            if all_boosts:
                total_spend = sum(boost['total_amount'] for boost in all_boosts)
                avg_spend = total_spend / len(all_boosts)
                high_spend_count = len([b for b in all_boosts if b['total_amount'] >= 500])
                
                boosted_data['marketing_insights'] = {
                    'total_tokens': len(all_boosts),
                    'total_marketing_spend': total_spend,
                    'average_spend': avg_spend,
                    'high_spend_tokens': high_spend_count,
                    'momentum': 'high' if avg_spend > 200 else 'medium' if avg_spend > 50 else 'low'
                }
            
            self.logger.debug(f"✅ DexScreener boosts: {len(all_boosts)} tokens, ${boosted_data['marketing_insights'].get('total_marketing_spend', 0):,.0f} total spend")
            
        except Exception as e:
            self.logger.error(f"Error in DexScreener boosted analysis: {e}")
        
        return boosted_data

    async def _get_dexscreener_marketing_orders(self, token_address: str) -> Dict[str, Any]:
        """
        NEW: Get detailed marketing order history for a specific token
        Provides comprehensive marketing investment intelligence that Birdeye doesn't have
        """
        marketing_data = {
            'total_marketing_spend': 0.0,
            'order_history': [],
            'campaign_types': [],
            'marketing_momentum': 'none',
            'recent_campaigns': []
        }
                ...
                async with session.get(url) as response:
                    if response.status == 200:
                        ...
                        for order in orders:
                            ...
                            # Track recent campaigns (last 7 days)
                            if timestamp > (current_time_ms - seven_days_ms):
                        ...
                        recent_spend = sum(c['amount'] for c in recent_campaigns)
                        if recent_spend >= 500 or len(recent_campaigns) >= 3:
                            momentum = 'high'
                        elif recent_spend >= 100 or len(recent_campaigns) >= 2:
                            momentum = 'medium'
                        elif recent_spend >= 50 or len(recent_campaigns) >= 1:
                            momentum = 'low'
                        else:
                            momentum = 'none'
                        ...
                        marketing_data.update({
                            ...
                            'marketing_momentum': momentum,
                            'recent_campaigns': recent_campaigns
                        })
                        
                        self.logger.debug(f"📊 Marketing orders for {token_address}: ${total_spend:.0f} total, momentum: {momentum}")
                        
        except Exception as e:
            self.logger.warning(f"Error fetching marketing orders for {token_address}: {e}")
    ...
    async def _enhance_token_with_trading_data(self, token_data: Dict[str, Any], token_address: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not enhanced_data:
                self.logger.warning(f"⚠️ Enhanced data fetcher returned no data for {token_address}, using DexScreener-first approach")
                return await self._dexscreener_first_enhancement(token_data, token_address)
            ...
            self.logger.warning("⚠️ Enhanced data fetcher not available, using DexScreener-first approach")
            return await self._dexscreener_first_enhancement(token_data, token_address)
        except Exception as e:
            self.logger.error(f"❌ Error in enhanced data enhancement for {token_address}: {e}, using DexScreener-first approach")
            return await self._dexscreener_first_enhancement(token_data, token_address)
    ...
    async def _dexscreener_first_enhancement(self, token_data: Dict[str, Any], token_address: str) -> Dict[str, Any]:
        """
        DexScreener-first approach: Use DexScreener as primary source, Birdeye only for specific gaps
        This is the new optimized approach that minimizes expensive Birdeye calls by 60-80%
        """
        # Step 1: Get comprehensive DexScreener data first
        dexscreener_data = await self._get_dexscreener_trading_data(token_address)
        
        if dexscreener_data:
            # Merge DexScreener data into token_data
            token_data.update({
                'volume_24h': dexscreener_data.get('volume_24h', 0),
                'volume_6h': dexscreener_data.get('volume_6h', 0),
                'volume_1h': dexscreener_data.get('volume_1h', 0),
                'volume_5m': dexscreener_data.get('volume_5m', 0),
                'trades_24h': dexscreener_data.get('trades_24h', 0),
                'trades_1h': dexscreener_data.get('trades_1h', 0),
                'buys_24h': dexscreener_data.get('buys_24h', 0),
                'sells_24h': dexscreener_data.get('sells_24h', 0),
                'price_change_5m': dexscreener_data.get('price_change_5m', 0),
                'price_change_1h': dexscreener_data.get('price_change_1h', 0),
                'price_change_24h': dexscreener_data.get('price_change_24h', 0),
                'trading_data_source': 'dexscreener_primary'
            })
            
            # Update key fields if DexScreener has better data
            if dexscreener_data.get('liquidity_usd', 0) > token_data.get('liquidity', 0):
                token_data['liquidity'] = dexscreener_data['liquidity_usd']
            
            if not token_data.get('market_cap') and dexscreener_data.get('market_cap'):
                token_data['market_cap'] = dexscreener_data['market_cap']
            
            # Assess if we need Birdeye for additional data
            data_quality = self._assess_dexscreener_data_quality(dexscreener_data)
            needs_birdeye = data_quality['completeness'] < 0.8 or data_quality['needs_enhancement']
            
            if needs_birdeye:
                self.logger.debug(f"🔄 DexScreener data sufficient but enhancing with selective Birdeye data for {token_address}")
                # Only enhance with specific Birdeye data if really needed
                token_data = await self._selective_birdeye_enhancement(token_data, token_address, data_quality['missing_fields'])
            else:
                self.logger.info(f"✅ DexScreener-only enhancement successful for {token_address} (completeness: {data_quality['completeness']:.1%})")
                
        else:
            self.logger.debug(f"⚠️ DexScreener data unavailable for {token_address}, using legacy Birdeye approach")
            # Fallback to legacy method only if DexScreener completely fails
            return await self._legacy_enhance_token_with_trading_data(token_data, token_address)
        
        return token_data

    def _assess_dexscreener_data_quality(self, dexscreener_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Assess DexScreener data quality to determine if Birdeye enhancement is needed
        """
        completeness_score = 0.0
        missing_fields = []
        
        # Essential data checks (70% of score)
        if dexscreener_data.get('volume_24h', 0) > 0:
            completeness_score += 0.25
        else:
            missing_fields.append('volume_data')
            
        if dexscreener_data.get('liquidity_usd', 0) > 0:
            completeness_score += 0.25
        else:
            missing_fields.append('liquidity_data')
            
        if dexscreener_data.get('trades_24h', 0) > 0:
            completeness_score += 0.2
        else:
            missing_fields.append('trading_activity')
        
        # Important additional data (30% of score)
        if dexscreener_data.get('price_change_24h') is not None:
            completeness_score += 0.1
        if dexscreener_data.get('volume_1h', 0) > 0:
            completeness_score += 0.1
        if dexscreener_data.get('buys_24h', 0) > 0:
            completeness_score += 0.1
        
        # Determine if enhancement needed
        needs_enhancement = (
            completeness_score < 0.8 or
            len(missing_fields) > 1 or
            'volume_data' in missing_fields
        )
        ...
        return {
            ...
            'quality_level': 'high' if completeness_score >= 0.8 else 'medium' if completeness_score >= 0.5 else 'low'
        }

    async def _selective_birdeye_enhancement(self, token_data: Dict[str, Any], token_address: str, missing_fields: List[str]) -> Dict[str, Any]:
        """
        Smart Birdeye routing based on token value and specific data needs
        High-value tokens get comprehensive Birdeye analysis, others get minimal/no Birdeye usage
        """
        try:
            # Determine token value tier for smart routing
            market_cap = token_data.get('market_cap', 0) or token_data.get('marketCap', 0)
            volume_24h = token_data.get('volume_24h', 0)
            liquidity = token_data.get('liquidity', 0) or token_data.get('liquidity_usd', 0)
            
            # Smart routing logic
            routing_decision = self._determine_birdeye_routing_tier(market_cap, volume_24h, liquidity)
            
            self.logger.info(f"🎯 Smart routing for {token_address}: {routing_decision['tier']} (MC:${market_cap:,.0f}, Vol:${volume_24h:,.0f})")
            
            # Route based on EARLY GEM OPTIMIZED tiers
            if routing_decision['tier'] == 'established':
                # Established tokens: DexScreener only (cost optimization)
                token_data = await self._minimal_birdeye_enhancement(token_data, token_address, missing_fields)
                
            elif routing_decision['tier'] == 'high_potential':
                # High potential early gems: Full Birdeye analysis
                token_data = await self._comprehensive_birdeye_enhancement(token_data, token_address, missing_fields)
                
            elif routing_decision['tier'] == 'emerging':
                # Emerging gems: Critical data only
                token_data = await self._critical_birdeye_enhancement(token_data, token_address, missing_fields)
                
            else:  # micro
                # Micro tokens: Light Birdeye analysis for early detection
                token_data = await self._light_birdeye_enhancement(token_data, token_address, missing_fields)
            
            # Track routing decision
            token_data['enhancement_method'] = f"smart_routing_{routing_decision['tier']}"
            token_data['routing_reasoning'] = routing_decision['reasoning']
            
        except Exception as e:
            self.logger.warning(f"Smart Birdeye routing failed for {token_address}: {e}")
            token_data['enhancement_method'] = 'dexscreener_only_fallback'
        
        return token_data

    def _determine_birdeye_routing_tier(self, market_cap: float, volume_24h: float, liquidity: float) -> Dict[str, Any]:
        """
        EARLY GEM OPTIMIZED Smart Routing - Designed specifically for early gem detection
        Lower thresholds to catch gems before they explode, while still optimizing costs
        """
        
        # MEGA-CAP: Established tokens (reduce Birdeye usage)
        if market_cap >= 1000000 or volume_24h >= 500000:
            return {
                'tier': 'established',
                'reasoning': f'Established token (MC:${market_cap:,.0f}, Vol:${volume_24h:,.0f}) - DexScreener sufficient'
            }
        
        # HIGH POTENTIAL: Strong early gems (full analysis)
        elif (market_cap >= 50000 and volume_24h >= 15000) or volume_24h >= 50000:
            return {
                'tier': 'high_potential', 
                'reasoning': f'High potential early gem (MC:${market_cap:,.0f}, Vol:${volume_24h:,.0f}) - full Birdeye analysis'
            }
        
        # EMERGING: Potential early gems (selective analysis)  
        elif (market_cap >= 10000 and volume_24h >= 5000) or liquidity >= 20000 or volume_24h >= 10000:
            return {
                'tier': 'emerging',
                'reasoning': f'Emerging gem candidate (MC:${market_cap:,.0f}, Vol:${volume_24h:,.0f}) - selective Birdeye analysis'
            }
        
        # MICRO: Very early or low activity (DexScreener primary, minimal Birdeye)
        else:
            return {
                'tier': 'micro',
                'reasoning': f'Micro/very early token (MC:${market_cap:,.0f}, Vol:${volume_24h:,.0f}) - cost-optimized analysis'
            }

    async def _comprehensive_birdeye_enhancement(self, token_data: Dict[str, Any], token_address: str, missing_fields: List[str]) -> Dict[str, Any]:
        """Full Birdeye analysis for high-potential early gems"""
        self.logger.info(f"💎 High-potential early gem {token_address}: Using comprehensive Birdeye analysis")
        
        try:
            # Get full token overview from Birdeye (justified for high-value tokens)
            if self.birdeye_api:
                birdeye_overview = await self.birdeye_api.get_token_overview(token_address)
                if birdeye_overview:
                    # Merge comprehensive Birdeye data
                    token_data.update({
                        'birdeye_price': birdeye_overview.get('price', 0),
                        'birdeye_market_cap': birdeye_overview.get('market_cap', 0),
                        'birdeye_volume_24h': birdeye_overview.get('volume_24h', 0),
                        'birdeye_price_change_24h': birdeye_overview.get('price_change_24h', 0),
                        'birdeye_liquidity': birdeye_overview.get('liquidity', 0),
                        'comprehensive_analysis': True
                    })
                    self.logger.info(f"✅ Comprehensive Birdeye data added for high-potential early gem {token_address}")
        
        except Exception as e:
            self.logger.error(f"Comprehensive Birdeye enhancement failed for {token_address}: {e}")
        
        return token_data

    async def _critical_birdeye_enhancement(self, token_data: Dict[str, Any], token_address: str, missing_fields: List[str]) -> Dict[str, Any]:
        """Critical data only for medium-value tokens"""
        self.logger.info(f"📊 Medium-value token {token_address}: Using selective Birdeye for critical gaps")
        
        try:
            # Only fetch critical missing data
            critical_fields = ['volume_data', 'liquidity_data'] 
            needs_critical_data = any(field in missing_fields for field in critical_fields)
            
            if needs_critical_data and self.birdeye_api:
                # Make targeted call only if critical data is missing
                birdeye_data = await self.birdeye_api.get_token_overview(token_address)
                if birdeye_data:
                    # Only update critical missing fields
                    if 'volume_data' in missing_fields:
                        token_data['birdeye_volume_24h'] = birdeye_data.get('volume_24h', 0)
                    if 'liquidity_data' in missing_fields:
                        token_data['birdeye_liquidity'] = birdeye_data.get('liquidity', 0)
                        
                    self.logger.debug(f"✅ Critical Birdeye data updated for medium-value token {token_address}")
        
        except Exception as e:
            self.logger.warning(f"Critical Birdeye enhancement failed for {token_address}: {e}")
        
        return token_data

    async def _minimal_birdeye_enhancement(self, token_data: Dict[str, Any], token_address: str, missing_fields: List[str]) -> Dict[str, Any]:
        """Minimal/no Birdeye usage for low-value tokens"""
        self.logger.debug(f"🪙 Low-value token {token_address}: Skipping Birdeye to preserve API costs")
        
        # For low-value tokens, rely on DexScreener data only
        # This maximizes cost efficiency for tokens that don't justify expensive API calls
        token_data.update({
            'cost_optimization': 'birdeye_skipped_for_low_value',
            'enhancement_rationale': 'DexScreener-only analysis sufficient for low-value token'
        })
        
        return token_data

    async def _light_birdeye_enhancement(self, token_data: Dict[str, Any], token_address: str, missing_fields: List[str]) -> Dict[str, Any]:
        """
        Light Birdeye analysis for micro/very early tokens
        Balances early gem detection with cost optimization
        """
        self.logger.debug(f"🔍 Micro token {token_address}: Using light Birdeye analysis for early gem detection")
        
        try:
            # For micro tokens, we still want SOME Birdeye data to catch early gems
            # But we're strategic about what we fetch
            critical_early_signals = ['volume_data', 'price_momentum']
            needs_early_detection = any(field in missing_fields for field in critical_early_signals)
            
            if needs_early_detection and self.birdeye_api:
                # Light Birdeye call - just basic overview for early gem signals
                birdeye_data = await self.birdeye_api.get_token_overview(token_address)
                if birdeye_data:
                    # Focus on early gem indicators only
                    token_data.update({
                        'birdeye_volume_24h': birdeye_data.get('volume_24h', 0),
                        'birdeye_price_change_24h': birdeye_data.get('price_change_24h', 0),
                        'early_gem_signals': {
                            'volume_growth': birdeye_data.get('volume_24h', 0) > 1000,  # Minimum activity
                            'price_momentum': abs(birdeye_data.get('price_change_24h', 0)) > 5,  # Some price action
                            'has_liquidity': birdeye_data.get('liquidity', 0) > 5000  # Basic liquidity
                        }
                    })
                    self.logger.debug(f"✅ Light Birdeye analysis completed for micro token {token_address}")
            else:
                # Even for micro tokens, add basic early gem detection flags
                token_data.update({
                    'early_gem_signals': {
                        'dexscreener_only': True,
                        'cost_optimized': True
                    }
                })
        
        except Exception as e:
            self.logger.warning(f"Light Birdeye enhancement failed for {token_address}: {e}")
        
        token_data.update({
            'cost_optimization': 'light_birdeye_for_early_detection',
            'enhancement_rationale': 'Balanced approach: early gem detection with cost optimization'
        })
